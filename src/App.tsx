...
function SummaryScreen({
  ...
  const getSummaryText = (successCount: number, totalPlayed: number) => {
    ...
    switch (successCount) {
      case 0:
        return 'Похоже, SDLC для тебя пока как тёмный лес. Не переживай — каждый сеньор когда-то не знал, чем Planning отличается от Retrospective. Начни с основ!'
      case 1:
        return 'Ты сделал(а) первый шаг в мире SDLC! Один балл — это начало пути. Главное не сдаваться после первого же Code Review 😉'
      case 2:
        return 'Хороший старт! Ты уже понимаешь базовые этапы SDLC. Ещё немного практики — и будешь проходить весь цикл как по нотам.'
      case 3:
        return 'Ты в теме! Знаешь основные этапы SDLC, но есть куда расти. Может, стоит глубже погрузиться в CI/CD процессы?'
      case 4:
        return 'Отличный результат! Ты явно разбираешься в SDLC не понаслышке. Наверное, уже не раз проходил(а) весь цикл от идеи до прода?'
      case 5:
        return 'Пять из пяти! Ты настоящий(ая) мастер SDLC. Знаешь каждый этап, каждый процесс. Сезон кода точно для тебя! 🏆'
      default:
        return 'Невероятно! Ты превзошёл(ла) все ожидания. Такой результат говорит о том, что SDLC для тебя — не просто аббревиатура, а образ жизни!'
    }
  }
  ...
  return (
    <div className="screen summary-screen">
      ...
      <p className="subtitle">
        {playerName ? `${playerName}, ` : ''}вот как ты справился с играми от Nessy:
      </p>
